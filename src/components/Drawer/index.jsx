...
try {
  setIsLoading(true)
  const {data} = await axios.post("https://adda001e83c231e1.mokky.dev/orders", {
    items: cartItems
  });
  ...
  setIsOrderComplete(true)
   ...
   for (let i = 0; i < cartItems.length; i++) {
    ...
    await axios.delete("https://adda001e83c231e1.mokky.dev/cart" + item.id);
    await delay(1000)
   }
...

{
 ...
 <div className='d-flex flex-column flex'>
  ...
  <img src="/img/arrow.svg" alt="" />
   ...
   : 
  <Info title={isOrderComplete ? 'Заказ оформлен!' : 'Корзина пустая'} description={isOrderComplete ? `Ваш заказ #${orderId} скоро будет передан курьерской доставке` : 'Добавьте хотя бы одну пару кроссовок, чтобы сделать заказ.'} image={isOrderComplete ? "/img/complete.jpeg" : "/img/empty-cart.jpeg"} />
}
